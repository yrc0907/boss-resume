...
当前适配器配置位于 `src/content/adapters/`，每个平台一个文件，由 `index.ts` 统一注册并通过 `getPlatformAdapter(location.hostname)` 选择当前站点。Boss 额外使用 `page-bridge.ts` 在页面主世界只读观察岗位 GET 响应，再由 `boss-api.ts` 递归提取并按多身份键（`encryptJobId/securityId/lid`）优先、岗位名/公司签名兜底合并到 DOM 卡片。选择器采用稳定 class、`data-*` 属性和语义兜底的组合；平台改版时只调整对应适配器和 fixture。
