...
扩展可以读取岗位公开文本、人工触发懒加载、计算本地匹配分、单卡或批量加入候选队列、根据卡片文案保守推断入口类型、生成招呼语模板、复制招呼语、为候选选择本地简历版本、导出带平台和入口类型字段的 CSV、记录岗位已打开/已处理状态、显示当前平台健康状态、导出选择器命中诊断和记录状态。
...
当前适配器配置位于 `src/content/adapters/`，每个平台一个文件，由 `index.ts` 统一注册并通过 `getPlatformAdapter(location.hostname)` 选择当前站点。通用 `dom-parser.ts` 负责字段兜底、岗位名清洗和链接身份键提取；Boss 和猎聘额外使用 `page-bridge.ts` 在页面主世界只读观察岗位 GET 响应，再由 `boss-api.ts` / `platform-api.ts` 提取并按身份键优先、岗位名/公司签名兜底合并到 DOM 卡片。平台改版时只调整对应适配器和 fixture。
...
6. 对存在 SPA 路由的站点，必须配置岗位列表/详情白名单，非岗位路由不显示工具条。
7. 简历版本只保存用户自定义名称和选择关系，不保存简历文件内容或本地路径。
...
`pnpm test` 会先构建扩展，再执行接口解析和平台路由契约测试，最后检查 Manifest、桥接脚本和禁止外部发送的源码静态约束。
